...
// 自动登录 - 异步
export const autoLogin = createAsyncThunk<User|null>(
    'auth/autoLogin',
    async (_, { rejectWithValue }) => {
        try {
            ...
            return rejectWithValue(error instanceof Error ? error.message : 'Auto login failed') // 返回异步错误信息
        }
    }
)
...
const authSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(autoLogin.fulfilled, (state:AuthState, action:PayloadAction<User|null>) => {
                state.user = action.payload
                state.loading = false
            })
            .addCase(autoLogin.rejected, (state:AuthState, action:PayloadAction<string|unknown>) => {
                ...
                state.error = action.payload instanceof Error ? action.payload.message : 'Auto login failed';
                state.loading = false
            })
    }
})
